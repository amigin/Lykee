...
namespace LykkeMarketPlace.Hubs
{
    public class LkHub : Hub
    {
        ...
        public static async Task RefreshOrderBooks()
        {
            var orderBooks = await GetOrderBooks();
            var hub = GlobalHost.ConnectionManager.GetLkHub();
        ...
        private void SendAssets()
        {
            Clients.Caller.Assets(
                GlobalSettings.FinanceInstruments.Values.Select(
                    itm => new {id = itm.Id, b = itm.Base, q = itm.Quoting, acc = itm.Accuracy}));
        }
        ...
        public async Task RefreshOrderBooks(string currency)
        {
        ...
        public async Task DoMarketOrder(LkeHubDealModel model)
        {
            ...
            var asset = GlobalSettings.FindInstument(model.CurFrom, model.CurTo);
